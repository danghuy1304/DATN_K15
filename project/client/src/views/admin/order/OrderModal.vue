...
const handleSubmit = async () => {
    await orderStore.fetchUpdateOrder(props.orderId, {
            ...
            <div class="modal-body">
                <form @submit.prevent="handleSubmit" class="row g-3 form-group">
                    <div class="form-item col-md-6">
                        ...
                        <p>{{ orderData.fullName }} - {{ orderData.phoneNumber }}</p>
                        <p>{{ orderData.address }}</p>
                    </div>
                    <div class="form-item col-md-6">
                        <label for="coupon" class="form-label px-2">Mã giảm giá: </label>
                        <span>{{ orderData?.coupon ?? 'Không có' }}</span><br />
                        <label for="paymentMethod" class="form-label px-2">Phương thức thanh toán: </label>
                    ...
                    <div class="form-item col-md-12">
                        <div class="order-detail-body">
                            ...
                            <div class="line"></div>
                            <div v-for="item in orderData.orderDetails" :key="item?.id" class="detail-body-content">
                    ...
                    <div class="form-item col-md-6">
                        ...
                        <select :disabled="orderData.status === 'UNPAID' || orderData.status === 'CANCELLED'"
                            v-model="orderData.status" class="form-select" id="status">
                            <option hidden value="PENDDING">
                                Chờ xác nhận
                            </option>
                            <option hidden value="PAID">
                                Đã thanh toán
                            </option>
                            <option hidden value="UNPAID">
                                Chưa thanh toán
                            </option>
                            <option hidden value="CANCELLED">
                                Đã hủy
                            </option>
                    ...
                    <div class="form-item col-md-6" style="display: flex; align-items: center; gap: 30px;">
                        <div>
                            <label for="description" class="form-label">Tổng tiền</label>
                            <p> {{ $formatValue.formatMoney(orderData.totalMoney) }}</p>
                        </div>
                        <div>
                            <label for="description" class="form-label">Ngày đặt hàng</label>
                            <p> {{ $formatValue.formatDateTime(orderData.createdAt) }}</p>
                        </div>
                    </div>
