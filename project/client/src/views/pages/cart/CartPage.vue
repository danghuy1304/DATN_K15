            ...
            <div class="cart-info">
                <div class="cart-header">
                    <div class="title-name cart-name">
                        <b-checkbox v-model="selectAll" :indeterminate="indeterminate"
                ...
                <div class="cart-body">
                    <div class="cart-content" v-for="item in cartData" :key="item?.id">
                        ...
                        <div class="cart-content-price cart-price">
                            <p>
                                {{
                                    $formatValue.formatMoney(handleCaculatorPrice(item?.product?.price,
                        ...
                        <div class="cart-content-quantity cart-quantity">
                            <div class="select-quantity">
                                <b-button @click="reduce(item)" icon="fa-solid fa-minus" />
                                <input @change="validInputQuantity(item)" type="number" min="1"
                                    :max="item?.product?.quantity" v-model="item.quantity" />
                                <b-button @click="increment(item)" icon="fa-solid fa-plus" />
                            </div>
                            <p>Còn {{ item?.product?.quantity }} sản phẩm</p>
                        </div>
                        <div class="cart-content-total cart-total">
                            <p>{{ $formatValue.formatMoney(handleCaculatorPrice(item?.product?.price,
            ...
            <div class="cart-sell">
                <div class="cart-sell-voucher">
                    ...
                    <div class="voucher-content">
                        <b-input @keydown.enter="handleAddCoupon()" v-model="couponCode"
                            placeholder="Nhập mã khuyến mãi" />
                        <b-button @click="handleAddCoupon()" type="secondary" value="Áp dụng"
                ...
                <div class="cart-sell-buy-now">
                    <div class="buy-now-bill">
                        ...
                        <div class="bill-value">
                            ...
                            <p>- {{ $formatValue.formatMoney((totalMoney / ((100 - discount) / 100)) - totalMoney) }}
                            </p>
                        </div>
                    </div>
                    <div class="line"></div>
...
const increment = (item) => {
    item.quantity = item.quantity < item.product?.quantity ? item.quantity += 1 : item.product?.quantity;
    handleUpdateQuantityCart(item)
}
...
const handleCaculatorTotalMoney = _.debounce(async function () {
    await orderStore.fetchCalculateTotalMoney(
        cartSelected.value, discount.value
    );
...
const handleAddCoupon = async () => {
    ...
    const res = await couponStore.fetchUseCoupon(couponCode.value.toUpperCase());
    if (res) {
