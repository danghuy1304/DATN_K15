...
<template>
    <div class="product-tag__container">
        ...
        <div class="product-price">
            <p class="price-new">{{
                item?.discount > 0 ?
                    $formatValue.formatMoney((item?.price * (100 - (item?.discount)) / 100))
                    : $formatValue.formatMoney(item?.price)
            }}
            </p>
            <p class="price-old">{{ item?.discount > 0 ? $formatValue.formatMoney(item?.price) : '' }}</p>
        </div>
    </div>
</template>
